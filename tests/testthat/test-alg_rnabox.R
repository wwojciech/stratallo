...
test_that("rnabox is valid function", {
  expect_function(
    rnabox,
    args = c("n", "A", "bounds_inner", "bounds_outer", "cmp_inner", "cmp_outer")
  )
})
...
test_that("rnabox works well for m, (pop507)", {
  ...
  result <- lapply(n_m51, rnabox, A_pop507, bounds_outer = N_pop507)
  result_twin <- lapply(
    n_m51, rnabox, A_pop507, N_pop507,
    cmp_inner = leq, cmp_outer = geq
  )
  expect_identical(result, result_twin)
  expect_snapshot(result)
})
...
test_that("rnabox works well for m (alloc: no bound), H = 1", {
  result <- rnabox(101, A[1], bounds_outer = m[1])
  result_twin <- rnabox(
    101, A[1], m[1],
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for m (alloc: 1 bound), H = 1", {
  result <- rnabox(m[1], A[1], bounds_outer = m[1])
  result_twin <- rnabox(
    m[1], A[1], m[1],
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for m (alloc: no bounds), H = 4", {
  result <- rnabox(600, A, bounds_outer = m)
  result_twin <- rnabox(
    600, A, m,
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for m (alloc: 1 bound), H = 4", {
  result <- rnabox(463, A, bounds_outer = m)
  result_twin <- rnabox(
    463, A, m,
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for m (alloc: 2 bounds), H = 4", {
  result <- rnabox(375, A, bounds_outer = m)
  result_twin <- rnabox(
    375, A, m,
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for m (alloc: 3 bounds), H = 4", {
  result <- rnabox(338, A, bounds_outer = m)
  result_twin <- rnabox(
    338, A, m,
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for n = sum(m), H = 4", {
  result <- rnabox(310, A, bounds_outer = m)
  result_twin <- rnabox(
    310, A, m,
    cmp_inner = leq, cmp_outer = geq
  )
...
test_that("rnabox works well for M, (pop507)", {
  ...
  result_twin <- lapply(
    ...
    bounds_outer = N_pop507, cmp_inner = leq, cmp_outer = geq
  )
  expect_identical(result, result_twin)
  expect_snapshot(result)
})
...
test_that("rnabox works well for M (alloc: no bound), H = 1", {
  ...
  result_twin <- rnabox(
    99, A[1],
    bounds_outer = M[1], cmp_inner = leq, cmp_outer = geq
  )
  expect_identical(result, result_twin)
...
test_that("rnabox works well for M with Inf, H = 4", {
  ...
  result_twin <- rnabox(
    5000, A,
    bounds_outer = c(M[1:3], Inf), cmp_inner = leq, cmp_outer = geq
  )
  expect_identical(result, result_twin)
  expect_equal(result, c(M[1:3], 4100))
})
...
test_that("rnabox works well for M (alloc: no bounds), H = 4", {
  ...
  result_twin <- rnabox(
    190, A,
    bounds_outer = M, cmp_inner = leq, cmp_outer = geq
  )
