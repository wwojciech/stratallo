...
test_that("rna works well when no bounds, H = 1)", {
  ...
  # Check assignments.
  result_asgn <- rna(400, a[1], assignments = TRUE)
...
test_that("rna works well when no bounds, H = 4)", {
  ...
  # Check assignments.
  result_asgn <- rna(400, a, assignments = TRUE)
...
test_that("rna works well for m, (pop507)", {
  ...
  result <- lapply(n_m51, rna, a_pop507, N_pop507, FALSE)
  expect_snapshot(result)

  # Check assignments.
  result_asgn <- lapply(n_m51, rna, a_pop507, N_pop507, FALSE, TRUE)
...
test_that("rna works well for m (alloc: no bound), H = 1", {
  result <- rna(101, a[1], bounds[1], FALSE)
  expect_equal(result, 101)

  # Check assignments.
  result_asgn <- rna(101, a[1], bounds[1], FALSE, TRUE)
...
test_that("rna works well for m (alloc: 1 bound), H = 1", {
  result <- rna(bounds[1], a[1], bounds[1], FALSE)
  expect_equal(result, bounds[1])

  # Check assignments.
  result_asgn <- rna(bounds[1], a[1], bounds[1], FALSE, TRUE)
...
test_that("rna works well for m (alloc: 1 bound), H = 4", {
  result <- rna(500, a, bounds, FALSE)
  expected <- c(105, 140, 175, 80)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(500, a, bounds, FALSE, TRUE)
...
test_that("rna works well for m (alloc: 2 bounds), H = 4", {
  result <- rna(420, a, bounds, FALSE)
  expected <- c(100, 106.6667, 133.3333, 80)
  expect_equal(result, expected, tolerance = 1e-6)

  # Check assignments.
  result_asgn <- rna(420, a, bounds, FALSE, TRUE)
...
test_that("rna works well for m (alloc: 3 bounds), H = 4", {
  result <- rna(370, a, bounds, FALSE)
  expected <- c(100, 90, 100, 80)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(370, a, bounds, FALSE, TRUE)
...
test_that("rna works well for n = sum(m), H = 4", {
  result <- rna(340, a, bounds, FALSE)
  expected <- c(100, 90, 70, 80)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(340, a, bounds, FALSE, TRUE)
...
test_that("rna works well for M, (pop507)", {
  ...
  result <- lapply(n_M50, rna, a_pop507, N_pop507)
  expect_snapshot(result)

  # Check assignments.
  result_asgn <- lapply(n_M50, rna, a_pop507, N_pop507, assignments = TRUE)
  expect_snapshot(result_asgn)
})
...
test_that("rna works well for M (alloc: no bound), H = 1", {
  result <- rna(99, a[1], bounds[1])
  expect_equal(result, 99)

  # Check assignments.
  result_asgn <- rna(99, a[1], bounds[1], assignments = TRUE)
...
test_that("rna works well for M (alloc: 1 bound), H = 1", {
  result <- rna(bounds[1], a[1], bounds[1])
  expect_equal(result, bounds[1])

  # Check assignments.
  result_asgn <- rna(bounds[1], a[1], bounds[1], assignments = TRUE)
  expected_asgn <- list(opt = bounds[1], take_neyman = numeric(0), take_bound = 1)
  expect_equal(result_asgn, expected_asgn)
})
...
test_that("rna works well for M with Inf, H = 4", {
  result <- rna(5000, a, c(bounds[1:3], Inf))
  expected <- c(100, 90, 70, 4740)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(5000, a, c(bounds[1:3], Inf), assignments = TRUE)
...
test_that("rna works well for M (alloc: no bounds), H = 4", {
  result <- rna(190, a, bounds)
  expected <- c(40.71429, 54.28571, 67.85714, 27.14286)
  expect_equal(result, expected, tolerance = 1e-7)

  # Check assignments.
  result_asgn <- rna(190, a, bounds, assignments = TRUE)
  expected_asgn <- list(opt = expected, take_neyman = 1:4, take_bound = numeric(0))
  expect_equal(result_asgn, expected_asgn, tolerance = 1e-7)
})
...
test_that("rna works well for M (alloc: 1 bound), H = 4", {
  result <- rna(270, a, bounds)
  expected <- c(66.66667, 88.88889, 70, 44.44444)
  expect_equal(result, expected, tolerance = 1e-7)

  # Check assignments.
  result_asgn <- rna(270, a, bounds, assignments = TRUE)
...
test_that("rna works well for M (alloc: 2 bounds), H = 4", {
  result <- rna(300, a, bounds)
  expected <- c(84, 90, 70, 56)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(300, a, bounds, assignments = TRUE)
...
test_that("rna works well for M (alloc: 3 bounds), H = 4", {
  result <- rna(330, a, bounds)
  expected <- c(100, 90, 70, 70)
  expect_equal(result, expected)

  # Check assignments.
  result_asgn <- rna(330, a, bounds, assignments = TRUE)
  expected_asgn <- list(opt = expected, take_neyman = 4, take_bound = 1:3)
  expect_equal(result_asgn, expected_asgn)
})
...
test_that("rna works well for n = sum(M), H = 4", {
  result <- rna(340, a, bounds)
  expect_equal(result, bounds)

  # Check assignments.
  result_asgn <- rna(340, a, bounds, assignments = TRUE)
