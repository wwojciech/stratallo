...
The `dopt` function solves the following three types of the allocation problem, 
formulated in the language of mathematical optimization.

**Problem 1 (one-sided upper bounds constraints)**   
Given numbers $a_w > 0,\, M_w > 0,\, w \in \W$ and $b,\, n \le \sum_{w \in \W}\, M_w$,
\begin{align*}
...
Given numbers $a_w > 0,\, m_w > 0,\, w \in \W$, and $b,\, n \ge \sum_{w \in \W} m_w$,
\begin{align*}
	\underset{\x \in (0, +\infty)^{H}}{\texteq{minimize ~\,}} & \quad f(\x) = \sum_{w \in \W} \tfrac{a_w^2}{x_w} - b \\
	\texteq{subject ~ to} & \quad \sum_{w \in \W} x_w = n \\
	& \quad x_w \ge m_w, \quad \forall w \in \W,
\end{align*}
where $\x = (x_w)_{w \in \W}$ is the optimization variable. 

**Problem 3 (box-constraints)**   
Given numbers $a_w > 0,\, 0 < m_w < M_w,\, w \in \W$, and $b,\, \sum_{w \in \W} m_w \le n \le \sum_{w \in \W} M_w$,
\begin{align*}
	\underset{\x \in (0, +\infty)^{H}}{\texteq{minimize ~\,}} & \quad f(\x) = \sum_{w \in \W} \tfrac{a_w^2}{x_w} - b \\
	\texteq{subject ~ to} & \quad \sum_{w \in \W} x_w = n \\
	& \quad x_w \ge m_w, \quad \forall w \in \W, \\
...
use of `m` and `M` arguments of the function. In case of **Problem 1**, user
provides the values of upper bounds with `M` argument, while leaving `m` as `NULL`. 
Similarly, for **Problem 2**, user provides the values of lower bounds with `m`
argument, while leaving `M` as `NULL`. In case of **Problem 3**, both arguments
`m` and `M` must be specified. If both `m` and `M` are `NULL` (default), the
`dopt` returns the value of Tschuprov-Neyman allocation that minimizes variance
$D^2_{st}$ under constraints on total sample size
